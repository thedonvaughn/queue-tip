...
class AMI
  ...
  def initialize(args = {})
    ...
    @server = @config['queuetastic']['ast_server'] || "127.0.0.1"
    @port = @config['queuetastic']['ast_port'] || "5038" 
    @username = @config['queuetastic']['username'] || "username" 
    @secret = @config['queuetastic']['secret'] || "secret"
